...
st.text("Please choose an input for each feature. Depending on your input our model \nwill predict the probability of fraudulence.\n  \nYou can use left-sidebar to specify your inputs. ")
...
if predict:
    ...
    if result_prob[:,1][0]  < 0.5:
        st.success(result[0], icon="✅")
        st.write("You can relax!", "The probality of being fradual is:", np.round(np.round(result_prob[:,1][0], 3)*100,2), "percent.",  "This is a nonfradual transaction.")
    else:
        st.error(result[0], icon="🚨")
        st.write("Bad news!", "The probality of being fradual is:", np.round(np.round(result_prob[:,1][0], 3)*100,2), "percent.",  "This is a fradual transaction.")
